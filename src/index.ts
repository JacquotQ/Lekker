...
/**
 * Build an SSE response from a pre-generated dictionary entry.
 * Mimics the OpenAI-compatible SSE format so clients don't need changes.
 */
function dictToSSE(content: string): Response {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // ── 1. Check local dictionary in KV ──────────────────────
    const dictKey = `${req.language}:${req.query.toLowerCase().trim()}`;
    const cached = await env.DICT.get(dictKey);

    if (cached) {
      try {
        const entry: DictEntry = JSON.parse(cached);
        // Return pre-generated content as SSE (no rate limit consumed)
        return dictToSSE(entry.response);
      } catch {
        // Malformed KV entry — fall through to API
      }
    }

    // ── 2. Fallback: Qwen API (rate limited) ─────────────────
